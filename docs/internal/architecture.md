...
5. **画像の縮小は `src/lib/resize.ts` の段階的縮小を通す。** `drawImage` で一度に大きく縮小すると、縮小率に対して高周波成分が残りすぎてエイリアシングが起きる。細部が潰れるうえ、元画像に無い高周波ノイズが生まれて JPEG の圧縮効率も下がるため。目標の2倍を超える間は1/2ずつ縮小する。
