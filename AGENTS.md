...
テストの構成は [docs/internal/testing.md](docs/internal/testing.md) を参照。

## Validation

- 完了前に必ず `npm run format`・`npm run lint`・`npm test`・`npm run build`が通ることを確認すること。
- ブラウザ API に依存しないロジックを追加・変更したときは、対応するテストも追加すること。テストで守れる範囲は限られるため、これはブラウザでの動作確認の代わりにはならない。
