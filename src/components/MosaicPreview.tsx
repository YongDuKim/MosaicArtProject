  ...
  return (
    <div className="preview card">
      ...
      <div className="preview-image-wrap">
        <img className="preview-image" src={resultUrl} alt="生成されたモザイクアート" />
        {selectedTile && (
          <canvas
            ref={overlayRef}
      ...
      {selectedTile ? (
        ...
        <p className="preview-meta">
          {result.outputWidth.toLocaleString()} × {result.outputHeight.toLocaleString()} px /
          グリッド {result.gridWidth} × {result.gridHeight}
        </p>
      )}
      <button
        type="button"
        onClick={() => downloadBlob(result.blob, `mosaic_output_${formatTimestamp()}.png`)}
      >
        PNGをダウンロード
      </button>
    </div>
